...
def parse(string,grammar):
    ...
    try:
        while len(left)>0:
            ...
            loopcount+=1
            symbol = left.pop()
            print(symbol,string[i],loopcount,'loopcount')
            ...
            if isNonTerminal(symbol):
                for production in grammar[symbol]:
                    if check(string[i],production,grammar):
                        left += production[::-1]
                        #print('Production added to left',production)
                        break
                else:
                    print(symbol,string[i],'Exception1')
                    againError = True
                    raise Exception
            else:
                if symbol!=string[i]:
                    againError  = True
                    print(symbol,string[i],'Exception2')
                    raise Exception
                else:
                    i+=1
    except:
        # print('againerror',againError)
        # if againError:
        #     return False
